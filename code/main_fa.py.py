import tkinter as tk
from tkinter import ttk, messagebox, colorchooser
import json
import os
import sys
import jdatetime
import re
from collections import defaultdict
import hashlib

desktop = os.path.join(os.path.expanduser("~"), "Desktop")
DATA_FILE = os.path.join(desktop, "finance_data.json")
SETTING_FILE = os.path.join(desktop, "settings.json")
PASSWORD_FILE = os.path.join(desktop, "password.json")

DEFAULT_BG = "#FFE4E1"
DEFAULT_TABLE_BG = "#FFF0F5"
DEFAULT_BTN_GREEN = "#4CAF50"
DEFAULT_BTN_BLUE = "#2196F3"
DEFAULT_BTN_RED = "#F44336"
DEFAULT_BTN_ORANGE = "#FF9800"
DEFAULT_BTN_PURPLE = "#9C27B0"

def hash_password(password):
    return hashlib.sha256(password.encode()).hexdigest()

def load_password_data():
    if os.path.exists(PASSWORD_FILE):
        with open(PASSWORD_FILE, "r", encoding="utf-8") as f:
            return json.load(f)
    return None

def save_password_data(password_hash, question, answer_hash):
    data = {"password_hash": password_hash, "question": question, "answer_hash": answer_hash}
    with open(PASSWORD_FILE, "w", encoding="utf-8") as f:
        json.dump(data, f, ensure_ascii=False, indent=2)

def check_password(password):
    password_data = load_password_data()
    if not password_data:
        return True
    return hash_password(password) == password_data["password_hash"]

def load_settings():
    if os.path.exists(SETTING_FILE):
        with open(SETTING_FILE, "r", encoding="utf-8") as f:
            return json.load(f)
    return {
        "auto_date": True,
        "add_salary_to_balance": True,
        "bg_color": DEFAULT_BG,
        "table_bg": DEFAULT_TABLE_BG,
        "btn_green": DEFAULT_BTN_GREEN,
        "btn_blue": DEFAULT_BTN_BLUE,
        "btn_red": DEFAULT_BTN_RED,
        "btn_orange": DEFAULT_BTN_ORANGE,
        "btn_purple": DEFAULT_BTN_PURPLE
    }

def save_settings(settings):
    with open(SETTING_FILE, "w", encoding="utf-8") as f:
        json.dump(settings, f, ensure_ascii=False, indent=2)

settings = load_settings()

def get_persian_date():
    return jdatetime.date.today().strftime("%Y/%m/%d")

def convert_to_number(text):
    cleaned = re.sub(r'[^\d]', '', str(text))
    try:
        return int(cleaned) if cleaned else 0
    except:
        return 0

def load_data():
    if os.path.exists(DATA_FILE):
        with open(DATA_FILE, "r", encoding="utf-8") as f:
            data = json.load(f)
            if "balance" not in data:
                data["balance"] = 0
            if "initial_balance" not in data:
                data["initial_balance"] = 0
            if "monthly_balances" not in data:
                data["monthly_balances"] = {}
            if "notes" not in data:
                data["notes"] = {}
            return data
    return {
        "transactions": [],
        "salary": [],
        "savings": [],
        "charity": [],
        "debt_claim": [],
        "installments": [],
        "balance": 0,
        "initial_balance": 0,
        "monthly_balances": {},
        "notes": {}
    }

def save_data(data):
    with open(DATA_FILE, "w", encoding="utf-8") as f:
        json.dump(data, f, ensure_ascii=False, indent=2)

jalali_months = ["فروردین", "اردیبهشت", "خرداد", "تیر", "مرداد", "شهریور", "مهر", "آبان", "آذر", "دی", "بهمن", "اسفند"]
months_map = {name: i+1 for i, name in enumerate(jalali_months)}

en_to_fa_months = {
    "Farvardin": "فروردین", "Ordibehesht": "اردیبهشت", "Khordad": "خرداد",
    "Tir": "تیر", "Mordad": "مرداد", "Shahrivar": "شهریور",
    "Mehr": "مهر", "Aban": "آبان", "Azar": "آذر",
    "Dey": "دی", "Bahman": "بهمن", "Esfand": "اسفند"
}

def get_last_day_of_month(year, month):
    if month <= 6:
        return 31
    elif month <= 11:
        return 30
    else:
        leap_years = [1, 5, 9, 13, 17, 22, 26, 30]
        if (year % 33) in leap_years:
            return 30
        return 29

def get_month_key():
    return f"{filter_year.get()}_{filter_month.get()}"

def get_selected_date_range():
    year = int(filter_year.get())
    month_name = filter_month.get()
    if month_name in en_to_fa_months:
        month_name = en_to_fa_months[month_name]
    month_num = months_map[month_name]
    start_date = jdatetime.date(year, month_num, 1)
    last_day = get_last_day_of_month(year, month_num)
    end_date = jdatetime.date(year, month_num, last_day)
    return start_date, end_date

# ===== رمز عبور =====
def show_password_dialog():
    password_data = load_password_data()

    if not password_data:
        dialog = tk.Toplevel(root)
        dialog.title("تنظیم رمز عبور")
        dialog.geometry("500x500")
        dialog.configure(bg=settings["bg_color"])
        dialog.transient(root)
        dialog.grab_set()

        tk.Label(dialog, text="🔐 تنظیم رمز عبور", bg=settings["bg_color"],
                font=("Arial", 14, "bold")).pack(pady=20)
        tk.Label(dialog, text="رمز عبور جدید:", bg=settings["bg_color"], font=("Arial", 11)).pack(pady=5)
        password_entry = tk.Entry(dialog, show="*", width=30, font=("Arial", 11))
        password_entry.pack(pady=5)
        tk.Label(dialog, text="تکرار رمز عبور:", bg=settings["bg_color"], font=("Arial", 11)).pack(pady=5)
        confirm_entry = tk.Entry(dialog, show="*", width=30, font=("Arial", 11))
        confirm_entry.pack(pady=5)
        tk.Label(dialog, text="سوال امنیتی:", bg=settings["bg_color"], font=("Arial", 11)).pack(pady=5)
        question_entry = tk.Entry(dialog, width=30, font=("Arial", 11))
        question_entry.insert(0, "غذای مورد علاقه شما چیست؟")
        question_entry.pack(pady=5)
        tk.Label(dialog, text="پاسخ سوال امنیتی:", bg=settings["bg_color"], font=("Arial", 11)).pack(pady=5)
        answer_entry = tk.Entry(dialog, width=30, font=("Arial", 11))
        answer_entry.pack(pady=5)

        def save_password():
            password = password_entry.get().strip()
            confirm = confirm_entry.get().strip()
            question = question_entry.get().strip()
            answer = answer_entry.get().strip()
            if not password or not confirm:
                messagebox.showerror("خطا", "لطفاً رمز عبور را وارد کنید")
                return
            if password != confirm:
                messagebox.showerror("خطا", "رمز عبور و تکرار آن مطابقت ندارند")
                return
            if not question or not answer:
                messagebox.showerror("خطا", "لطفاً سوال و پاسخ امنیتی را وارد کنید")
                return
            save_password_data(hash_password(password), question, hash_password(answer))
            dialog.destroy()
            messagebox.showinfo("موفق", "رمز عبور با موفقیت تنظیم شد")

        tk.Button(dialog, text="تنظیم رمز عبور", command=save_password,
                 bg=settings["btn_green"], fg='white', font=("Arial", 11), padx=20, pady=5).pack(pady=20)
        root.wait_window(dialog)
        return load_password_data() is not None

    else:
        attempt = 0
        while attempt < 3:
            dialog = tk.Toplevel(root)
            dialog.title("ورود به برنامه")
            dialog.geometry("350x250")
            dialog.configure(bg=settings["bg_color"])
            dialog.transient(root)
            dialog.grab_set()

            tk.Label(dialog, text="🔐 ورود به برنامه", bg=settings["bg_color"],
                    font=("Arial", 14, "bold")).pack(pady=20)
            tk.Label(dialog, text="رمز عبور:", bg=settings["bg_color"], font=("Arial", 11)).pack(pady=5)
            password_entry = tk.Entry(dialog, show="*", width=30, font=("Arial", 11))
            password_entry.pack(pady=5)
            password_entry.focus()

            result = {"success": False}

            def check_entered_password():
                if check_password(password_entry.get().strip()):
                    result["success"] = True
                    dialog.destroy()
                else:
                    messagebox.showerror("خطا", "رمز عبور اشتباه است")
                    password_entry.delete(0, tk.END)
                    password_entry.focus()

            def forgot_password():
                dialog.destroy()
                show_forgot_password_dialog()

            btn_frame = tk.Frame(dialog, bg=settings["bg_color"])
            btn_frame.pack(pady=20)
            tk.Button(btn_frame, text="ورود", command=check_entered_password,
                     bg=settings["btn_green"], fg='white', font=("Arial", 11), padx=20, pady=5).pack(side=tk.LEFT, padx=10)
            tk.Button(btn_frame, text="فراموشی رمز", command=forgot_password,
                     bg=settings["btn_red"], fg='white', font=("Arial", 10), padx=15, pady=5).pack(side=tk.LEFT, padx=10)
            dialog.bind("<Return>", lambda e: check_entered_password())
            root.wait_window(dialog)

            if result["success"]:
                return True
            attempt += 1

        messagebox.showerror("خطا", "تعداد دفعات مجاز ورود به پایان رسید")
        return False

def show_forgot_password_dialog():
    password_data = load_password_data()
    if not password_data:
        messagebox.showerror("خطا", "سیستم رمز عبور تنظیم نشده است")
        return

    dialog = tk.Toplevel(root)
    dialog.title("بازیابی رمز عبور")
    dialog.geometry("400x300")
    dialog.configure(bg=settings["bg_color"])
    dialog.transient(root)
    dialog.grab_set()

    tk.Label(dialog, text="❓ بازیابی رمز عبور", bg=settings["bg_color"],
            font=("Arial", 14, "bold")).pack(pady=20)
    tk.Label(dialog, text=f"سوال امنیتی: {password_data['question']}",
            bg=settings["bg_color"], font=("Arial", 11)).pack(pady=10)
    tk.Label(dialog, text="پاسخ:", bg=settings["bg_color"], font=("Arial", 11)).pack(pady=5)
    answer_entry = tk.Entry(dialog, width=30, font=("Arial", 11))
    answer_entry.pack(pady=5)
    answer_entry.focus()

    def set_new_password():
        answer = answer_entry.get().strip()
        if hash_password(answer) == password_data["answer_hash"]:
            dialog.destroy()
            change_password_dialog(force_change=True)
        else:
            messagebox.showerror("خطا", "پاسخ اشتباه است")
            answer_entry.delete(0, tk.END)
            answer_entry.focus()

    btn_frame = tk.Frame(dialog, bg=settings["bg_color"])
    btn_frame.pack(pady=20)
    tk.Button(btn_frame, text="✅ تأیید پاسخ", command=set_new_password,
             bg=settings["btn_green"], fg='white', font=("Arial", 11), padx=20, pady=5).pack(side=tk.LEFT, padx=10)
    tk.Button(btn_frame, text="❌ لغو", command=dialog.destroy,
             bg=settings["btn_red"], fg='white', font=("Arial", 10), padx=15, pady=5).pack(side=tk.LEFT, padx=10)
    dialog.bind("<Return>", lambda e: set_new_password())
    root.wait_window(dialog)

def change_password_dialog(force_change=False):
    dialog = tk.Toplevel(root)
    dialog.title("تغییر رمز عبور")
    dialog.geometry("400x380")
    dialog.configure(bg=settings["bg_color"])
    dialog.transient(root)
    dialog.grab_set()

    tk.Label(dialog, text="🔑 تغییر رمز عبور", bg=settings["bg_color"],
            font=("Arial", 14, "bold")).pack(pady=20)

    old_pass_entry = None
    if not force_change:
        tk.Label(dialog, text="رمز عبور فعلی:", bg=settings["bg_color"], font=("Arial", 11)).pack(pady=5)
        old_pass_entry = tk.Entry(dialog, show="*", width=30, font=("Arial", 11))
        old_pass_entry.pack(pady=5)

    tk.Label(dialog, text="رمز عبور جدید:", bg=settings["bg_color"], font=("Arial", 11)).pack(pady=5)
    new_pass_entry = tk.Entry(dialog, show="*", width=30, font=("Arial", 11))
    new_pass_entry.pack(pady=5)
    tk.Label(dialog, text="تکرار رمز عبور جدید:", bg=settings["bg_color"], font=("Arial", 11)).pack(pady=5)
    confirm_entry = tk.Entry(dialog, show="*", width=30, font=("Arial", 11))
    confirm_entry.pack(pady=5)
    tk.Label(dialog, text="سوال امنیتی جدید:", bg=settings["bg_color"], font=("Arial", 11)).pack(pady=5)
    question_entry = tk.Entry(dialog, width=30, font=("Arial", 11))
    password_data = load_password_data()
    question_entry.insert(0, password_data["question"] if password_data else "نام مادر شما چیست؟")
    question_entry.pack(pady=5)
    tk.Label(dialog, text="پاسخ جدید:", bg=settings["bg_color"], font=("Arial", 11)).pack(pady=5)
    answer_entry = tk.Entry(dialog, width=30, font=("Arial", 11))
    answer_entry.pack(pady=5)

    def save_new_password():
        if not force_change:
            pd = load_password_data()
            if not pd or hash_password(old_pass_entry.get().strip()) != pd["password_hash"]:
                messagebox.showerror("خطا", "رمز عبور فعلی اشتباه است")
                return
        new_pass = new_pass_entry.get().strip()
        confirm = confirm_entry.get().strip()
        question = question_entry.get().strip()
        answer = answer_entry.get().strip()
        if not new_pass or not confirm:
            messagebox.showerror("خطا", "لطفاً رمز عبور جدید را وارد کنید")
            return
        if new_pass != confirm:
            messagebox.showerror("خطا", "رمز عبور و تکرار آن مطابقت ندارند")
            return
        if not question or not answer:
            messagebox.showerror("خطا", "لطفاً سوال و پاسخ امنیتی را وارد کنید")
            return
        save_password_data(hash_password(new_pass), question, hash_password(answer))
        dialog.destroy()
        messagebox.showinfo("موفق", "رمز عبور با موفقیت تغییر یافت")

    tk.Button(dialog, text="تغییر رمز عبور", command=save_new_password,
             bg=settings["btn_green"], fg='white', font=("Arial", 11), padx=20, pady=5).pack(pady=20)

# ===== شروع برنامه =====
root = tk.Tk()
root.tk.call('tk', 'scaling', 2)
root.title("مدیریت مالی")
root.geometry("1300x950")
root.configure(bg=settings["bg_color"])
root.resizable(True, True)

if not show_password_dialog():
    root.destroy()
    sys.exit()

style = ttk.Style()
style.configure("TNotebook", background=settings["bg_color"])
style.configure("TFrame", background=settings["bg_color"])
style.configure("Treeview", background=settings["table_bg"], fieldbackground=settings["table_bg"], font=("Arial", 10))
style.configure("Treeview.Heading", font=("Arial", 11, "bold"))
style.configure("Treeview", rowheight=30)

data = load_data()

top_frame = tk.Frame(root, bg=settings["bg_color"])
top_frame.pack(fill=tk.X, padx=10, pady=5)

balance_frame = tk.Frame(top_frame, bg=settings["bg_color"], relief=tk.RIDGE, bd=2)
balance_frame.pack(fill=tk.X, padx=10, pady=5)

lbl_balance = tk.Label(balance_frame, text="💰 موجودی فعلی: 0 تومان",
                       bg=settings["bg_color"], font=("Arial", 14, "bold"), fg=settings["btn_green"])
lbl_balance.pack(padx=20, pady=5)

# ===== منطق موجودی =====
def get_current_month_balance():
    key = get_month_key()
    return data["monthly_balances"].get(key, 0)

def set_current_month_balance(amount):
    key = get_month_key()
    data["monthly_balances"][key] = amount
    save_data(data)

def update_balance():
    balance = get_current_month_balance()
    lbl_balance.config(text=f"💰 موجودی فعلی: {balance:,} تومان")
    return balance

def set_initial_balance():
    dialog = tk.Toplevel(root)
    dialog.title("تنظیم موجودی")
    dialog.geometry("300x150")
    dialog.configure(bg=settings["bg_color"])
    dialog.transient(root)
    dialog.grab_set()
    tk.Label(dialog, text="موجودی خود را وارد کنید:", bg=settings["bg_color"], font=("Arial", 11)).pack(pady=10)
    entry_balance = tk.Entry(dialog, font=("Arial", 11), width=20)
    entry_balance.pack(pady=5)

    def save_initial():
        try:
            amount = convert_to_number(entry_balance.get())
            set_current_month_balance(amount)
            update_balance()
            save_data(data)
            dialog.destroy()
            messagebox.showinfo("موفق", "موجودی ثبت شد")
        except:
            messagebox.showerror("خطا", "مقدار عددی رو درست وارد کن")

    tk.Button(dialog, text="ثبت", command=save_initial, bg=settings["btn_green"], fg='white').pack(pady=10)

tk.Button(balance_frame, text="تنظیم موجودی اولیه", command=set_initial_balance,
          bg=settings["btn_blue"], fg='white', font=("Arial", 10)).pack(padx=20, pady=5)

def apply_filter():
    update_transactions()
    update_salary_list()
    update_savings_list()
    update_charity_list()
    update_debt_claim_list()
    update_monthly_report()
    update_yearly_report()
    update_installments_list()
    update_notes()
    update_balance()

filter_frame = tk.Frame(top_frame, bg=settings["bg_color"])
filter_frame.pack(side=tk.RIGHT, padx=10)

tk.Label(filter_frame, text="فیلتر بر اساس ماه:", bg=settings["bg_color"], font=("Arial", 11, "bold")).pack(side=tk.LEFT, padx=5)
tk.Label(filter_frame, text="ماه:", bg=settings["bg_color"]).pack(side=tk.LEFT, padx=2)
filter_month = ttk.Combobox(filter_frame, values=jalali_months, width=8)
current_month_fa = en_to_fa_months.get(jdatetime.date.today().strftime("%B"), "فروردین")
filter_month.set(current_month_fa)
filter_month.pack(side=tk.LEFT, padx=2)
filter_month.bind("<<ComboboxSelected>>", lambda e: apply_filter())

tk.Label(filter_frame, text="سال:", bg=settings["bg_color"]).pack(side=tk.LEFT, padx=2)
filter_year = ttk.Combobox(filter_frame, values=[str(i) for i in range(1400, 1410)], width=6)
filter_year.set(str(jdatetime.date.today().year))
filter_year.pack(side=tk.LEFT, padx=2)
filter_year.bind("<<ComboboxSelected>>", lambda e: apply_filter())

tk.Button(filter_frame, text="🔄 بروزرسانی", command=apply_filter, bg=settings["btn_orange"], fg='white', font=("Arial", 10)).pack(side=tk.LEFT, padx=10)

notebook = ttk.Notebook(root, padding=10)
notebook.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)

def select_date(title="انتخاب تاریخ", default_date=None):
    result = {"date": None, "success": False}

    dialog = tk.Toplevel(root)
    dialog.title(title)
    dialog.geometry("300x280")
    dialog.configure(bg=settings["bg_color"])
    dialog.transient(root)
    dialog.grab_set()

    tk.Label(dialog, text="تاریخ را انتخاب کنید:", bg=settings["bg_color"], font=("Arial", 12, "bold")).pack(pady=10)

    today = jdatetime.date.today()
    default_day = today.strftime("%d")
    default_month = en_to_fa_months.get(today.strftime("%B"), "فروردین")
    default_year = str(today.year)

    frame_day = tk.Frame(dialog, bg=settings["bg_color"])
    frame_day.pack(pady=5)
    tk.Label(frame_day, text="روز:", bg=settings["bg_color"], font=("Arial", 11)).pack(side=tk.LEFT, padx=5)
    day_spin = ttk.Combobox(frame_day, values=[f"{i:02d}" for i in range(1, 32)], width=5)
    day_spin.set(default_day)
    day_spin.pack(side=tk.LEFT)

    frame_month = tk.Frame(dialog, bg=settings["bg_color"])
    frame_month.pack(pady=5)
    tk.Label(frame_month, text="ماه:", bg=settings["bg_color"], font=("Arial", 11)).pack(side=tk.LEFT, padx=5)
    month_combo = ttk.Combobox(frame_month, values=jalali_months, width=10)
    month_combo.set(default_month)
    month_combo.pack(side=tk.LEFT)

    frame_year = tk.Frame(dialog, bg=settings["bg_color"])
    frame_year.pack(pady=5)
    tk.Label(frame_year, text="سال:", bg=settings["bg_color"], font=("Arial", 11)).pack(side=tk.LEFT, padx=5)
    year_combo = ttk.Combobox(frame_year, values=[str(i) for i in range(1400, 1410)], width=6)
    year_combo.set(default_year)
    year_combo.pack(side=tk.LEFT)

    def confirm():
        try:
            year = int(year_combo.get())
            month_name = month_combo.get()
            if month_name in en_to_fa_months:
                month_name = en_to_fa_months[month_name]
            month_num = months_map[month_name]
            day = int(day_spin.get())
            selected_date = jdatetime.date(year, month_num, day)
            result["date"] = selected_date.strftime("%Y/%m/%d")
            result["success"] = True
            dialog.destroy()
        except Exception as e:
            messagebox.showerror("خطا", f"تاریخ نامعتبر است: {e}")

    tk.Button(dialog, text="✅ تأیید", command=confirm,
              bg=settings["btn_green"], fg='white', font=("Arial", 11), padx=20, pady=5).pack(pady=20)

    root.wait_window(dialog)
    return result["date"] if result["success"] else None

def _in_range(date_str, start, end):
    try:
        y, m, d = map(int, date_str.split('/'))
        return start <= jdatetime.date(y, m, d) <= end
    except:
        return False

# ===== خرید و خرج =====
frame_trans = tk.Frame(notebook, bg=settings["bg_color"])
notebook.add(frame_trans, text="📋 خرید و خرج")

form_frame = tk.Frame(frame_trans, bg=settings["bg_color"])
form_frame.pack(pady=10)

tk.Label(form_frame, text="عنوان:", bg=settings["bg_color"], font=("Arial", 11)).grid(row=0, column=0, padx=10, pady=5)
entry_title = tk.Entry(form_frame, width=30, font=("Arial", 11))
entry_title.grid(row=0, column=1, padx=10, pady=5)

tk.Label(form_frame, text="مبلغ:", bg=settings["bg_color"], font=("Arial", 11)).grid(row=0, column=2, padx=10, pady=5)
entry_amount = tk.Entry(form_frame, width=20, font=("Arial", 11))
entry_amount.grid(row=0, column=3, padx=10, pady=5)

def add_transaction():
    title = entry_title.get().strip()
    if not title:
        messagebox.showerror("خطا", "عنوان رو وارد کن")
        return
    amount = convert_to_number(entry_amount.get())

    selected_date = select_date("انتخاب تاریخ تراکنش")
    if selected_date is None:
        return

    data["transactions"].append({"title": title, "amount": amount, "date": selected_date})
    balance = get_current_month_balance() - amount
    set_current_month_balance(balance)
    save_data(data)

    entry_title.delete(0, tk.END)
    entry_amount.delete(0, tk.END)
    update_transactions()
    update_monthly_report()
    update_yearly_report()
    update_balance()
    messagebox.showinfo("موفق", f"تراکنش برای تاریخ {selected_date} ثبت شد")

tk.Button(form_frame, text="➕ اضافه کن", command=add_transaction, bg=settings["btn_green"], fg='white', font=("Arial", 11)).grid(row=1, column=0, columnspan=4, pady=10)

columns_trans = ("تاریخ", "عنوان", "مبلغ (تومان)")
tree_trans = ttk.Treeview(frame_trans, columns=columns_trans, show="headings", height=8)
for col in columns_trans:
    tree_trans.heading(col, text=col)
tree_trans.column("تاریخ", width=110, anchor="center")
tree_trans.column("عنوان", width=380, anchor="center")
tree_trans.column("مبلغ (تومان)", width=160, anchor="center")
tree_trans.pack(fill=tk.BOTH, expand=True, padx=10, pady=5)

trans_btn_frame = tk.Frame(frame_trans, bg=settings["bg_color"])
trans_btn_frame.pack(pady=5)
tk.Button(trans_btn_frame, text="✏️ ویرایش ردیف", command=lambda: edit_selected_transaction(),
          bg=settings["btn_orange"], fg='white', font=("Arial", 10)).pack(side=tk.LEFT, padx=5)
tk.Button(trans_btn_frame, text="🗑 حذف ردیف",
          command=lambda: deleted_selected(tree_trans, data["transactions"], "transaction"),
          bg=settings["btn_red"], fg='white', font=("Arial", 10)).pack(side=tk.LEFT, padx=5)

report_frame = tk.Frame(frame_trans, bg=settings["bg_color"])
report_frame.pack(fill=tk.X, padx=10, pady=5)
tk.Label(report_frame, text="📊 گزارش ماهانه:", bg=settings["bg_color"], font=("Arial", 12, "bold")).pack(side=tk.LEFT, padx=5)
monthly_report_text = tk.Text(report_frame, height=6, width=60, font=("Arial", 10), bg=settings["table_bg"])
monthly_report_text.pack(side=tk.LEFT, padx=10, fill=tk.BOTH, expand=True)

def update_monthly_report():
    monthly_report_text.delete(1.0, tk.END)
    monthly_expense = defaultdict(int)
    monthly_income = defaultdict(int)

    for t in data["transactions"]:
        try:
            y, m, d = map(int, t["date"].split('/'))
            monthly_expense[f"{y} {jalali_months[m-1]}"] += t["amount"]
        except:
            pass

    for s in data["salary"]:
        try:
            y, m, d = map(int, s["date"].split('/'))
            monthly_income[f"{y} {jalali_months[m-1]}"] += s["amount"]
        except:
            pass

    report = "گزارش مالی ماهانه:\n" + "-" * 50 + "\n"
    all_months = set(monthly_expense.keys()) | set(monthly_income.keys())
    for month in sorted(all_months):
        income = monthly_income.get(month, 0)
        expense = monthly_expense.get(month, 0)
        net = income - expense
        report += f"{month}:\n   حقوق: {income:,} تومان\n   خرج: {expense:,} تومان\n   مانده: {net:,} تومان\n\n"

    if not all_months:
        report += "هیچ داده‌ای موجود نیست"
    monthly_report_text.insert(1.0, report)

def update_transactions():
    for row in tree_trans.get_children():
        tree_trans.delete(row)

    start_date, end_date = get_selected_date_range()
    filtered = [t for t in data["transactions"] if _in_range(t["date"], start_date, end_date)]
    filtered.sort(key=lambda x: x["date"], reverse=True)

    for t in filtered:
        tree_trans.insert("", tk.END, values=(t["date"], t["title"], f"{t['amount']:,}"))

def edit_selected_transaction():
    selected = tree_trans.selection()
    if not selected:
        messagebox.showerror("خطا", "لطفاً یک ردیف انتخاب کنید")
        return
    item = tree_trans.item(selected[0])
    values = item['values']
    index = tree_trans.index(selected[0])
    actual_index = -(index + 1)

    dialog = tk.Toplevel(root)
    dialog.title("ویرایش تراکنش")
    dialog.geometry("400x300")
    dialog.configure(bg=settings["bg_color"])
    dialog.transient(root)
    dialog.grab_set()

    tk.Label(dialog, text="عنوان:", bg=settings["bg_color"], font=("Arial", 11)).grid(row=0, column=0, padx=10, pady=10, sticky='e')
    entry_edit_title = tk.Entry(dialog, width=30, font=("Arial", 11))
    entry_edit_title.insert(0, values[1])
    entry_edit_title.grid(row=0, column=1, padx=10, pady=10)

    tk.Label(dialog, text="مبلغ:", bg=settings["bg_color"], font=("Arial", 11)).grid(row=1, column=0, padx=10, pady=10, sticky='e')
    entry_edit_amount = tk.Entry(dialog, width=20, font=("Arial", 11))
    entry_edit_amount.insert(0, values[2].replace(",", ""))
    entry_edit_amount.grid(row=1, column=1, padx=10, pady=10)

    tk.Label(dialog, text="تاریخ:", bg=settings["bg_color"], font=("Arial", 11)).grid(row=2, column=0, padx=10, pady=10, sticky='e')
    entry_edit_date = tk.Entry(dialog, width=20, font=("Arial", 11))
    entry_edit_date.insert(0, values[0])
    entry_edit_date.grid(row=2, column=1, padx=10, pady=10)

    def save_edit():
        new_title = entry_edit_title.get().strip()
        if not new_title:
            messagebox.showerror("خطا", "عنوان رو وارد کن")
            return
        new_amount = convert_to_number(entry_edit_amount.get())
        new_date = entry_edit_date.get().strip()
        old_amount = data["transactions"][actual_index]["amount"]
        data["transactions"][actual_index] = {"title": new_title, "amount": new_amount, "date": new_date}

        balance = get_current_month_balance() + old_amount - new_amount
        set_current_month_balance(balance)
        save_data(data)

        update_transactions()
        update_monthly_report()
        update_yearly_report()
        update_balance()
        dialog.destroy()
        messagebox.showinfo("موفق", "ویرایش شد")

    tk.Button(dialog, text="ذخیره تغییرات", command=save_edit, bg=settings["btn_green"], fg='white', font=("Arial", 11)).grid(row=3, column=0, columnspan=2, pady=20)

# ===== حقوق =====
frame_salary = tk.Frame(notebook, bg=settings["bg_color"])
notebook.add(frame_salary, text="💰 حقوق")

salary_form = tk.Frame(frame_salary, bg=settings["bg_color"])
salary_form.pack(pady=10)

tk.Label(salary_form, text="منبع:", bg=settings["bg_color"], font=("Arial", 11)).grid(row=0, column=0, padx=10, pady=5)
entry_salary_source = tk.Entry(salary_form, width=25, font=("Arial", 11))
entry_salary_source.grid(row=0, column=1, padx=10, pady=5)

tk.Label(salary_form, text="مبلغ:", bg=settings["bg_color"], font=("Arial", 11)).grid(row=0, column=2, padx=10, pady=5)
entry_salary = tk.Entry(salary_form, width=20, font=("Arial", 11))
entry_salary.grid(row=0, column=3, padx=10, pady=5)

def add_salary():
    source = entry_salary_source.get().strip()
    if not source:
        messagebox.showerror("خطا", "منبع رو وارد کن")
        return
    amount = convert_to_number(entry_salary.get())

    selected_date = select_date("انتخاب تاریخ حقوق")
    if selected_date is None:
        return

    data["salary"].append({"source": source, "amount": amount, "date": selected_date})
    balance = get_current_month_balance() + amount
    set_current_month_balance(balance)
    save_data(data)

    entry_salary_source.delete(0, tk.END)
    entry_salary.delete(0, tk.END)
    update_salary_list()
    update_monthly_report()
    update_yearly_report()
    update_balance()
    messagebox.showinfo("موفق", f"حقوق برای تاریخ {selected_date} ثبت شد")

tk.Button(salary_form, text="ثبت حقوق", command=add_salary, bg=settings["btn_blue"], fg='white', font=("Arial", 11)).grid(row=1, column=0, columnspan=4, pady=10)

columns_salary = ("تاریخ", "منبع", "مبلغ (تومان)")
tree_salary = ttk.Treeview(frame_salary, columns=columns_salary, show="headings", height=8)
for col in columns_salary:
    tree_salary.heading(col, text=col)
tree_salary.column("تاریخ", width=100, anchor="center")
tree_salary.column("منبع", width=200, anchor="center")
tree_salary.column("مبلغ (تومان)", width=150, anchor="center")
tree_salary.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)

total_salary_frame = tk.Frame(frame_salary, bg=settings["bg_color"])
total_salary_frame.pack(fill=tk.X, padx=10, pady=5)
lbl_total_salary = tk.Label(total_salary_frame, text="", bg=settings["bg_color"], font=("Arial", 12, "bold"), fg=settings["btn_blue"])
lbl_total_salary.pack()

salary_btn_frame = tk.Frame(frame_salary, bg=settings["bg_color"])
salary_btn_frame.pack(pady=5)
tk.Button(salary_btn_frame, text="✏️ ویرایش ردیف", command=lambda: edit_selected_salary(),
          bg=settings["btn_orange"], fg='white', font=("Arial", 10)).pack(side=tk.LEFT, padx=5)
tk.Button(salary_btn_frame, text="🗑 حذف ردیف",
          command=lambda: deleted_selected(tree_salary, data["salary"], "salary"),
          bg=settings["btn_red"], fg='white', font=("Arial", 10)).pack(side=tk.LEFT, padx=5)

def update_salary_list():
    for row in tree_salary.get_children():
        tree_salary.delete(row)

    start_date, end_date = get_selected_date_range()
    filtered = [s for s in data["salary"] if _in_range(s["date"], start_date, end_date)]
    filtered.sort(key=lambda x: x["date"], reverse=True)

    total = sum(s["amount"] for s in filtered)
    for s in filtered:
        tree_salary.insert("", tk.END, values=(s["date"], s["source"], f"{s['amount']:,}"))
    lbl_total_salary.config(text=f"مجموع حقوق: {total:,} تومان")

def edit_selected_salary():
    selected = tree_salary.selection()
    if not selected:
        messagebox.showerror("خطا", "لطفاً یک ردیف را انتخاب کنید")
        return
    item = tree_salary.item(selected[0])
    values = item['values']
    index = tree_salary.index(selected[0])
    actual_index = -(index + 1)

    dialog = tk.Toplevel(root)
    dialog.title("ویرایش حقوق")
    dialog.geometry("400x250")
    dialog.configure(bg=settings["bg_color"])
    dialog.transient(root)
    dialog.grab_set()

    tk.Label(dialog, text="منبع:", bg=settings["bg_color"], font=("Arial", 11)).grid(row=0, column=0, padx=10, pady=10, sticky='e')
    entry_edit_source = tk.Entry(dialog, width=30, font=("Arial", 11))
    entry_edit_source.insert(0, values[1])
    entry_edit_source.grid(row=0, column=1, padx=10, pady=10)

    tk.Label(dialog, text="مبلغ:", bg=settings["bg_color"], font=("Arial", 11)).grid(row=1, column=0, padx=10, pady=10, sticky='e')
    entry_edit_amount = tk.Entry(dialog, width=20, font=("Arial", 11))
    entry_edit_amount.insert(0, values[2].replace(",", ""))
    entry_edit_amount.grid(row=1, column=1, padx=10, pady=10)

    tk.Label(dialog, text="تاریخ:", bg=settings["bg_color"], font=("Arial", 11)).grid(row=2, column=0, padx=10, pady=10, sticky='e')
    entry_edit_date = tk.Entry(dialog, width=20, font=("Arial", 11))
    entry_edit_date.insert(0, values[0])
    entry_edit_date.grid(row=2, column=1, padx=10, pady=10)

    def save_edit():
        new_source = entry_edit_source.get().strip()
        if not new_source:
            messagebox.showerror("خطا", "منبع رو وارد کن")
            return
        new_amount = convert_to_number(entry_edit_amount.get())
        new_date = entry_edit_date.get().strip()
        old_amount = data["salary"][actual_index]["amount"]
        data["salary"][actual_index] = {"source": new_source, "amount": new_amount, "date": new_date}

        balance = get_current_month_balance() - old_amount + new_amount
        set_current_month_balance(balance)
        save_data(data)

        update_salary_list()
        update_monthly_report()
        update_yearly_report()
        update_balance()
        dialog.destroy()
        messagebox.showinfo("موفق", "ویرایش شد")

    tk.Button(dialog, text="ذخیره تغییرات", command=save_edit, bg=settings["btn_green"], fg='white', font=("Arial", 11)).grid(row=3, column=0, columnspan=2, pady=20)

# ===== پس‌انداز =====
frame_savings = tk.Frame(notebook, bg=settings["bg_color"])
notebook.add(frame_savings, text="🏦 پس‌انداز")

savings_form = tk.Frame(frame_savings, bg=settings["bg_color"])
savings_form.pack(pady=10)

tk.Label(savings_form, text="مبلغ:", bg=settings["bg_color"], font=("Arial", 11)).grid(row=0, column=0, padx=10, pady=5)
entry_savings = tk.Entry(savings_form, width=20, font=("Arial", 11))
entry_savings.grid(row=0, column=1, padx=10, pady=5)

def add_savings():
    amount = convert_to_number(entry_savings.get())
    if amount == 0:
        messagebox.showerror("خطا", "لطفاً مبلغ معتبر وارد کنید")
        return

    selected_date = select_date("انتخاب تاریخ پس‌انداز")
    if selected_date is None:
        return

    data["savings"].append({"amount": amount, "date": selected_date})
    balance = get_current_month_balance() - amount
    set_current_month_balance(balance)
    save_data(data)

    entry_savings.delete(0, tk.END)
    update_savings_list()
    update_yearly_report()
    update_balance()
    messagebox.showinfo("موفق", f"پس‌انداز برای تاریخ {selected_date} ثبت شد")

tk.Button(savings_form, text="ثبت پس‌انداز", command=add_savings, bg=settings["btn_green"], fg='white', font=("Arial", 11)).grid(row=0, column=2, padx=10, pady=5)

columns_savings = ("تاریخ", "مبلغ (تومان)")
tree_savings = ttk.Treeview(frame_savings, columns=columns_savings, show="headings", height=8)
for col in columns_savings:
    tree_savings.heading(col, text=col)
tree_savings.column("تاریخ", width=150, anchor="center")
tree_savings.column("مبلغ (تومان)", width=150, anchor="center")
tree_savings.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)

total_savings_frame = tk.Frame(frame_savings, bg=settings["bg_color"])
total_savings_frame.pack(fill=tk.X, padx=10, pady=5)
lbl_total_savings = tk.Label(total_savings_frame, text="", bg=settings["bg_color"], font=("Arial", 12, "bold"), fg=settings["btn_green"])
lbl_total_savings.pack()

savings_btn_frame = tk.Frame(frame_savings, bg=settings["bg_color"])
savings_btn_frame.pack(pady=5)
tk.Button(savings_btn_frame, text="✏️ ویرایش ردیف", command=lambda: edit_selected_savings(),
          bg=settings["btn_orange"], fg='white', font=("Arial", 10)).pack(side=tk.LEFT, padx=5)
tk.Button(savings_btn_frame, text="🗑 حذف ردیف",
          command=lambda: deleted_selected(tree_savings, data["savings"], "savings"),
          bg=settings["btn_red"], fg='white', font=("Arial", 10)).pack(side=tk.LEFT, padx=5)

def update_savings_list():
    for row in tree_savings.get_children():
        tree_savings.delete(row)

    start_date, end_date = get_selected_date_range()
    filtered = [s for s in data["savings"] if _in_range(s["date"], start_date, end_date)]
    filtered.sort(key=lambda x: x["date"], reverse=True)

    total = sum(s["amount"] for s in filtered)
    for s in filtered:
        tree_savings.insert("", tk.END, values=(s["date"], f"{s['amount']:,}"))
    lbl_total_savings.config(text=f"مجموع پس‌انداز: {total:,} تومان")

def edit_selected_savings():
    selected = tree_savings.selection()
    if not selected:
        messagebox.showerror("خطا", "لطفاً یک ردیف را انتخاب کنید")
        return
    item = tree_savings.item(selected[0])
    values = item['values']
    index = tree_savings.index(selected[0])
    actual_index = -(index + 1)

    dialog = tk.Toplevel(root)
    dialog.title("ویرایش پس‌انداز")
    dialog.geometry("350x200")
    dialog.configure(bg=settings["bg_color"])
    dialog.transient(root)
    dialog.grab_set()

    tk.Label(dialog, text="مبلغ:", bg=settings["bg_color"], font=("Arial", 11)).grid(row=0, column=0, padx=10, pady=10, sticky='e')
    entry_edit_amount = tk.Entry(dialog, width=20, font=("Arial", 11))
    entry_edit_amount.insert(0, values[1].replace(",", ""))
    entry_edit_amount.grid(row=0, column=1, padx=10, pady=10)

    tk.Label(dialog, text="تاریخ:", bg=settings["bg_color"], font=("Arial", 11)).grid(row=1, column=0, padx=10, pady=10, sticky='e')
    entry_edit_date = tk.Entry(dialog, width=20, font=("Arial", 11))
    entry_edit_date.insert(0, values[0])
    entry_edit_date.grid(row=1, column=1, padx=10, pady=10)

    def save_edit():
        new_amount = convert_to_number(entry_edit_amount.get())
        new_date = entry_edit_date.get().strip()
        old_amount = data["savings"][actual_index]["amount"]
        data["savings"][actual_index] = {"amount": new_amount, "date": new_date}

        balance = get_current_month_balance() + old_amount - new_amount
        set_current_month_balance(balance)
        save_data(data)

        update_savings_list()
        update_yearly_report()
        update_balance()
        dialog.destroy()
        messagebox.showinfo("موفق", "ویرایش شد")

    tk.Button(dialog, text="ذخیره تغییرات", command=save_edit, bg=settings["btn_green"], fg='white', font=("Arial", 11)).grid(row=2, column=0, columnspan=2, pady=20)

# ===== کار خیر =====
frame_charity = tk.Frame(notebook, bg=settings["bg_color"])
notebook.add(frame_charity, text="🤝 کار خیر")

charity_form = tk.Frame(frame_charity, bg=settings["bg_color"])
charity_form.pack(pady=10)

tk.Label(charity_form, text="نام خیریه:", bg=settings["bg_color"], font=("Arial", 11)).grid(row=0, column=0, padx=10, pady=5)
entry_charity_name = tk.Entry(charity_form, width=30, font=("Arial", 11))
entry_charity_name.grid(row=0, column=1, padx=10, pady=5)

tk.Label(charity_form, text="مبلغ:", bg=settings["bg_color"], font=("Arial", 11)).grid(row=0, column=2, padx=10, pady=5)
entry_charity_amount = tk.Entry(charity_form, width=20, font=("Arial", 11))
entry_charity_amount.grid(row=0, column=3, padx=10, pady=5)

def add_charity():
    name = entry_charity_name.get().strip()
    if not name:
        messagebox.showerror("خطا", "نام خیریه رو وارد کن")
        return
    amount = convert_to_number(entry_charity_amount.get())

    selected_date = select_date("انتخاب تاریخ کمک خیریه")
    if selected_date is None:
        return

    data["charity"].append({"name": name, "amount": amount, "date": selected_date})
    balance = get_current_month_balance() - amount
    set_current_month_balance(balance)
    save_data(data)

    entry_charity_name.delete(0, tk.END)
    entry_charity_amount.delete(0, tk.END)
    update_charity_list()
    update_yearly_report()
    update_balance()
    messagebox.showinfo("موفق", f"کمک خیریه برای تاریخ {selected_date} ثبت شد")

tk.Button(charity_form, text="➕ ثبت کمک", command=add_charity, bg=settings["btn_red"], fg='white', font=("Arial", 11)).grid(row=1, column=0, columnspan=4, pady=10)

columns_charity = ("تاریخ", "نام خیریه", "مبلغ (تومان)")
tree_charity = ttk.Treeview(frame_charity, columns=columns_charity, show="headings", height=8)
for col in columns_charity:
    tree_charity.heading(col, text=col)
tree_charity.column("تاریخ", width=100)
tree_charity.column("نام خیریه", width=250)
tree_charity.column("مبلغ (تومان)", width=150)
tree_charity.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)

total_charity_frame = tk.Frame(frame_charity, bg=settings["bg_color"])
total_charity_frame.pack(fill=tk.X, padx=10, pady=5)
lbl_total_charity = tk.Label(total_charity_frame, text="", bg=settings["bg_color"], font=("Arial", 12, "bold"), fg=settings["btn_red"])
lbl_total_charity.pack()

charity_btn_frame = tk.Frame(frame_charity, bg=settings["bg_color"])
charity_btn_frame.pack(pady=5)
tk.Button(charity_btn_frame, text="✏️ ویرایش ردیف", command=lambda: edit_selected_charity(),
          bg=settings["btn_orange"], fg='white', font=("Arial", 10)).pack(side=tk.LEFT, padx=5)
tk.Button(charity_btn_frame, text="🗑 حذف ردیف",
          command=lambda: deleted_selected(tree_charity, data["charity"], "charity"),
          bg=settings["btn_red"], fg='white', font=("Arial", 10)).pack(side=tk.LEFT, padx=5)

def update_charity_list():
    for row in tree_charity.get_children():
        tree_charity.delete(row)

    start_date, end_date = get_selected_date_range()
    filtered = [c for c in data["charity"] if _in_range(c["date"], start_date, end_date)]
    filtered.sort(key=lambda x: x["date"], reverse=True)

    total = sum(c["amount"] for c in filtered)
    for c in filtered:
        tree_charity.insert("", tk.END, values=(c["date"], c["name"], f"{c['amount']:,}"))
    lbl_total_charity.config(text=f"مجموع کمک‌ها: {total:,} تومان")

def edit_selected_charity():
    selected = tree_charity.selection()
    if not selected:
        messagebox.showerror("خطا", "لطفاً یک ردیف را انتخاب کنید")
        return
    item = tree_charity.item(selected[0])
    values = item['values']
    index = tree_charity.index(selected[0])
    actual_index = -(index + 1)

    dialog = tk.Toplevel(root)
    dialog.title("ویرایش کمک")
    dialog.geometry("400x250")
    dialog.configure(bg=settings["bg_color"])
    dialog.transient(root)
    dialog.grab_set()

    tk.Label(dialog, text="نام خیریه:", bg=settings["bg_color"], font=("Arial", 11)).grid(row=0, column=0, padx=10, pady=10, sticky='e')
    entry_edit_name = tk.Entry(dialog, width=30, font=("Arial", 11))
    entry_edit_name.insert(0, values[1])
    entry_edit_name.grid(row=0, column=1, padx=10, pady=10)

    tk.Label(dialog, text="مبلغ:", bg=settings["bg_color"], font=("Arial", 11)).grid(row=1, column=0, padx=10, pady=10, sticky='e')
    entry_edit_amount = tk.Entry(dialog, width=20, font=("Arial", 11))
    entry_edit_amount.insert(0, values[2].replace(",", ""))
    entry_edit_amount.grid(row=1, column=1, padx=10, pady=10)

    tk.Label(dialog, text="تاریخ:", bg=settings["bg_color"], font=("Arial", 11)).grid(row=2, column=0, padx=10, pady=10, sticky='e')
    entry_edit_date = tk.Entry(dialog, width=20, font=("Arial", 11))
    entry_edit_date.insert(0, values[0])
    entry_edit_date.grid(row=2, column=1, padx=10, pady=10)

    def save_edit():
        new_name = entry_edit_name.get().strip()
        if not new_name:
            messagebox.showerror("خطا", "نام خیریه رو وارد کن")
            return
        new_amount = convert_to_number(entry_edit_amount.get())
        new_date = entry_edit_date.get().strip()
        old_amount = data["charity"][actual_index]["amount"]
        data["charity"][actual_index] = {"name": new_name, "amount": new_amount, "date": new_date}

        balance = get_current_month_balance() + old_amount - new_amount
        set_current_month_balance(balance)
        save_data(data)

        update_charity_list()
        update_yearly_report()
        update_balance()
        dialog.destroy()
        messagebox.showinfo("موفق", "ویرایش شد")

    tk.Button(dialog, text="ذخیره تغییرات", command=save_edit, bg=settings["btn_green"], fg='white', font=("Arial", 11)).grid(row=3, column=0, columnspan=2, pady=20)

# ===== بدهکار و طلبکار =====
frame_dc = tk.Frame(notebook, bg=settings["bg_color"])
notebook.add(frame_dc, text="💰 بدهکار و طلبکار")

dc_form = tk.Frame(frame_dc, bg=settings["bg_color"])
dc_form.pack(pady=10)

tk.Label(dc_form, text="نوع:", bg=settings["bg_color"], font=("Arial", 11)).grid(row=0, column=0, padx=10, pady=5)
dc_type_var = tk.StringVar(value="بدهکار")
ttk.Combobox(dc_form, textvariable=dc_type_var, values=["بدهکار", "طلبکار"], width=10).grid(row=0, column=1, padx=10, pady=5)

tk.Label(dc_form, text="نام شخص:", bg=settings["bg_color"], font=("Arial", 11)).grid(row=0, column=2, padx=10, pady=5)
entry_person = tk.Entry(dc_form, width=20, font=("Arial", 11))
entry_person.grid(row=0, column=3, padx=10, pady=5)

tk.Label(dc_form, text="مبلغ:", bg=settings["bg_color"], font=("Arial", 11)).grid(row=1, column=0, padx=10, pady=5)
entry_dc_amount = tk.Entry(dc_form, width=20, font=("Arial", 11))
entry_dc_amount.grid(row=1, column=1, padx=10, pady=5)

tk.Label(dc_form, text="توضیحات:", bg=settings["bg_color"], font=("Arial", 11)).grid(row=1, column=2, padx=10, pady=5)
entry_desc = tk.Entry(dc_form, width=30, font=("Arial", 11))
entry_desc.grid(row=1, column=3, padx=10, pady=5)

def add_debt_claim():
    person = entry_person.get().strip()
    if not person:
        messagebox.showerror("خطا", "نام شخص رو وارد کن")
        return
    amount = convert_to_number(entry_dc_amount.get())

    selected_date = select_date("انتخاب تاریخ بدهکاری/طلبکاری")
    if selected_date is None:
        return

    data["debt_claim"].append({
        "type": dc_type_var.get(),
        "person": person,
        "amount": amount,
        "description": entry_desc.get().strip(),
        "date": selected_date,
        "settled": False
    })
    save_data(data)

    entry_person.delete(0, tk.END)
    entry_dc_amount.delete(0, tk.END)
    entry_desc.delete(0, tk.END)
    update_debt_claim_list()
    messagebox.showinfo("موفق", f"{dc_type_var.get()} برای تاریخ {selected_date} ثبت شد. پس از تسویه از موجودی کسر می‌شود.")

tk.Button(dc_form, text="➕ ثبت", command=add_debt_claim, bg=settings["btn_purple"], fg='white', font=("Arial", 11)).grid(row=2, column=0, columnspan=4, pady=10)

columns_dc = ("تاریخ", "نوع", "شخص", "مبلغ (تومان)", "توضیحات", "تسویه")
tree_dc = ttk.Treeview(frame_dc, columns=columns_dc, show="headings", height=8)
for col in columns_dc:
    tree_dc.heading(col, text=col)
tree_dc.column("تاریخ", width=100, anchor="center")
tree_dc.column("نوع", width=80, anchor="center")
tree_dc.column("شخص", width=120, anchor="center")
tree_dc.column("مبلغ (تومان)", width=120, anchor="center")
tree_dc.column("توضیحات", width=180, anchor="center")
tree_dc.column("تسویه", width=80, anchor="center")
tree_dc.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)

total_dc_frame = tk.Frame(frame_dc, bg=settings["bg_color"])
total_dc_frame.pack(fill=tk.X, padx=10, pady=5)
lbl_total_debt = tk.Label(total_dc_frame, text="", bg=settings["bg_color"], font=("Arial", 11, "bold"), fg=settings["btn_red"])
lbl_total_debt.pack(side=tk.LEFT, padx=10)
lbl_total_claim = tk.Label(total_dc_frame, text="", bg=settings["bg_color"], font=("Arial", 11, "bold"), fg=settings["btn_green"])
lbl_total_claim.pack(side=tk.LEFT, padx=10)

dc_btn_frame = tk.Frame(frame_dc, bg=settings["bg_color"])
dc_btn_frame.pack(pady=5)
tk.Button(dc_btn_frame, text="✏️ ویرایش ردیف", command=lambda: edit_selected_dc(),
          bg=settings["btn_orange"], fg='white', font=("Arial", 10)).pack(side=tk.LEFT, padx=5)
tk.Button(dc_btn_frame, text="🗑 حذف ردیف",
          command=lambda: deleted_selected(tree_dc, data["debt_claim"], None),
          bg=settings["btn_red"], fg='white', font=("Arial", 10)).pack(side=tk.LEFT, padx=5)
tk.Button(dc_btn_frame, text="✅ تسویه بدهکاری", command=lambda: toggle_settle_debt(tree_dc),
          bg=settings["btn_green"], fg='white', font=("Arial", 10)).pack(side=tk.LEFT, padx=5)

def toggle_settle_debt(tree):
    selected = tree.selection()
    if not selected:
        messagebox.showerror("خطا", "لطفاً یک ردیف انتخاب کنید")
        return

    index = tree.index(selected[0])
    actual_index = -(index + 1)
    current = data["debt_claim"][actual_index]

    if current["type"] == "طلبکار":
        messagebox.showwarning("غیرمجاز", "این مورد طلبکار است، فقط بدهکار قابل تسویه است.")
        return

    if not current["settled"]:
        current["settled"] = True
        balance = get_current_month_balance() - current["amount"]
        set_current_month_balance(balance)
        save_data(data)
        update_debt_claim_list()
        update_balance()
        messagebox.showinfo("تسویه", f"بدهکاری {current['person']} به مبلغ {current['amount']:,} تومان تسویه شد.")
    else:
        messagebox.showwarning("تکرار", "این بدهکاری قبلاً تسویه شده.")

def update_debt_claim_list():
    for row in tree_dc.get_children():
        tree_dc.delete(row)

    start_date, end_date = get_selected_date_range()
    filtered = [i for i in data["debt_claim"] if _in_range(i["date"], start_date, end_date)]
    filtered.sort(key=lambda x: x["date"], reverse=True)

    total_debt = sum(i["amount"] for i in filtered if i["type"] == "بدهکار")
    total_claim = sum(i["amount"] for i in filtered if i["type"] == "طلبکار")

    for item in filtered:
        settled_text = "✅" if item.get("settled", False) else "❌"
        tree_dc.insert("", tk.END, values=(
            item["date"], item["type"], item["person"],
            f"{item['amount']:,}", item.get("description", ""), settled_text
        ))
    lbl_total_debt.config(text=f"مجموع بدهکاران: {total_debt:,} تومان")
    lbl_total_claim.config(text=f"مجموع طلبکاران: {total_claim:,} تومان")

def edit_selected_dc():
    selected = tree_dc.selection()
    if not selected:
        messagebox.showerror("خطا", "لطفاً یک ردیف را انتخاب کنید")
        return
    item = tree_dc.item(selected[0])
    values = item['values']
    index = tree_dc.index(selected[0])
    actual_index = -(index + 1)

    dialog = tk.Toplevel(root)
    dialog.title("ویرایش")
    dialog.geometry("450x300")
    dialog.configure(bg=settings["bg_color"])
    dialog.transient(root)
    dialog.grab_set()

    tk.Label(dialog, text="نوع:", bg=settings["bg_color"], font=("Arial", 11)).grid(row=0, column=0, padx=10, pady=10, sticky='e')
    type_var = tk.StringVar(value=values[1])
    ttk.Combobox(dialog, textvariable=type_var, values=["بدهکار", "طلبکار"], width=15).grid(row=0, column=1, padx=10, pady=10)

    tk.Label(dialog, text="نام شخص:", bg=settings["bg_color"], font=("Arial", 11)).grid(row=1, column=0, padx=10, pady=10, sticky='e')
    entry_edit_person = tk.Entry(dialog, width=25, font=("Arial", 11))
    entry_edit_person.insert(0, values[2])
    entry_edit_person.grid(row=1, column=1, padx=10, pady=10)

    tk.Label(dialog, text="مبلغ:", bg=settings["bg_color"], font=("Arial", 11)).grid(row=2, column=0, padx=10, pady=10, sticky='e')
    entry_edit_amount = tk.Entry(dialog, width=20, font=("Arial", 11))
    entry_edit_amount.insert(0, values[3].replace(",", ""))
    entry_edit_amount.grid(row=2, column=1, padx=10, pady=10)

    tk.Label(dialog, text="توضیحات:", bg=settings["bg_color"], font=("Arial", 11)).grid(row=3, column=0, padx=10, pady=10, sticky='e')
    entry_edit_desc = tk.Entry(dialog, width=30, font=("Arial", 11))
    entry_edit_desc.insert(0, values[4])
    entry_edit_desc.grid(row=3, column=1, padx=10, pady=10)

    tk.Label(dialog, text="تاریخ:", bg=settings["bg_color"], font=("Arial", 11)).grid(row=4, column=0, padx=10, pady=10, sticky='e')
    entry_edit_date = tk.Entry(dialog, width=20, font=("Arial", 11))
    entry_edit_date.insert(0, values[0])
    entry_edit_date.grid(row=4, column=1, padx=10, pady=10)

    def save_edit():
        data["debt_claim"][actual_index] = {
            "type": type_var.get(),
            "person": entry_edit_person.get().strip(),
            "amount": convert_to_number(entry_edit_amount.get()),
            "description": entry_edit_desc.get().strip(),
            "date": entry_edit_date.get().strip(),
            "settled": data["debt_claim"][actual_index].get("settled", False)
        }
        save_data(data)
        update_debt_claim_list()
        update_balance()
        dialog.destroy()
        messagebox.showinfo("موفق", "ویرایش شد")

    tk.Button(dialog, text="ذخیره تغییرات", command=save_edit, bg=settings["btn_green"], fg='white', font=("Arial", 11)).grid(row=5, column=0, columnspan=2, pady=20)

# ===== اقساط =====
frame_installment = tk.Frame(notebook, bg=settings["bg_color"])
notebook.add(frame_installment, text="📅 اقساط")

installment_form = tk.Frame(frame_installment, bg=settings["bg_color"])
installment_form.pack(pady=10)

tk.Label(installment_form, text="عنوان قسط:", bg=settings["bg_color"], font=("Arial", 11)).grid(row=0, column=0, padx=10, pady=5)
entry_installment_title = tk.Entry(installment_form, width=25, font=("Arial", 11))
entry_installment_title.grid(row=0, column=1, padx=10, pady=5)

tk.Label(installment_form, text="مبلغ:", bg=settings["bg_color"], font=("Arial", 11)).grid(row=0, column=2, padx=10, pady=5)
entry_installment_amount = tk.Entry(installment_form, width=15, font=("Arial", 11))
entry_installment_amount.grid(row=0, column=3, padx=10, pady=5)

tk.Label(installment_form, text="تاریخ سررسید:", bg=settings["bg_color"], font=("Arial", 11)).grid(row=1, column=0, padx=10, pady=5)
tk.Label(installment_form, text="روز:", bg=settings["bg_color"]).grid(row=1, column=1, padx=5, sticky='e')
installment_day = ttk.Combobox(installment_form, values=[f"{i:02d}" for i in range(1, 32)], width=5)
installment_day.set(jdatetime.date.today().strftime("%d"))
installment_day.grid(row=1, column=1, padx=40, pady=5)

tk.Label(installment_form, text="ماه:", bg=settings["bg_color"]).grid(row=1, column=2, padx=5, sticky='e')
installment_month = ttk.Combobox(installment_form, values=jalali_months, width=8)
installment_month.set(current_month_fa)
installment_month.grid(row=1, column=2, padx=40, pady=5)

tk.Label(installment_form, text="سال:", bg=settings["bg_color"]).grid(row=1, column=3, padx=5, sticky='e')
installment_year = ttk.Combobox(installment_form, values=[str(i) for i in range(1400, 1410)], width=6)
installment_year.set(str(jdatetime.date.today().year))
installment_year.grid(row=1, column=3, padx=40, pady=5)

def update_installments_list():
    for row in tree_installment.get_children():
        tree_installment.delete(row)

    start_date, end_date = get_selected_date_range()
    for inst in data["installments"]:
        if not _in_range(inst["due_date"], start_date, end_date):
            continue
        status = "پرداخت شده ✅" if inst["is_paid"] else "پرداخت نشده ❌"
        paid_date = inst["paid_date"] if inst["paid_date"] else "---"
        tree_installment.insert("", tk.END, values=(
            inst["title"], f"{inst['amount']:,}", inst["due_date"], status, paid_date
        ))

def _get_selected_installment():
    selected = tree_installment.selection()
    if not selected:
        return None, None
    values = tree_installment.item(selected[0])["values"]
    due_date_in_row = values[2]
    title_in_row = values[0]
    for i, inst in enumerate(data["installments"]):
        if inst["title"] == title_in_row and inst["due_date"] == due_date_in_row:
            return i, inst
    return None, None

def add_installment():
    title = entry_installment_title.get().strip()
    if not title:
        messagebox.showerror("خطا", "لطفاً عنوان قسط را وارد کنید")
        return
    amount = convert_to_number(entry_installment_amount.get())
    if amount == 0:
        messagebox.showerror("خطا", "لطفاً مبلغ معتبر وارد کنید")
        return

    year = int(installment_year.get())
    month_name = installment_month.get()
    if month_name in en_to_fa_months:
        month_name = en_to_fa_months[month_name]
    month_num = months_map[month_name]
    day = int(installment_day.get())

    try:
        due_date_str = jdatetime.date(year, month_num, day).strftime("%Y/%m/%d")
    except:
        messagebox.showerror("خطا", "تاریخ وارد شده نامعتبر است")
        return

    data["installments"].append({
        "title": title, "amount": amount, "due_date": due_date_str, "is_paid": False, "paid_date": None
    })
    save_data(data)
    entry_installment_title.delete(0, tk.END)
    entry_installment_amount.delete(0, tk.END)
    update_installments_list()
    messagebox.showinfo("موفق", f"قسط '{title}' با مبلغ {amount:,} تومان و سررسید {due_date_str} ثبت شد")

def mark_installment_paid():
    selected = tree_installment.selection()
    if not selected:
        messagebox.showerror("خطا", "لطفاً یک قسط را انتخاب کنید")
        return

    real_index, inst = _get_selected_installment()
    if real_index is None:
        return

    if inst["is_paid"]:
        messagebox.showwarning("تکرار", "این قسط قبلاً پرداخت شده است")
        return

    inst["is_paid"] = True
    inst["paid_date"] = get_persian_date()

    balance = get_current_month_balance() - inst["amount"]
    set_current_month_balance(balance)
    save_data(data)

    update_installments_list()
    update_balance()
    messagebox.showinfo("موفق", f"قسط {inst['title']} به مبلغ {inst['amount']:,} تومان پرداخت شد")

def delete_installment():
    selected = tree_installment.selection()
    if not selected:
        messagebox.showerror("خطا", "لطفاً یک قسط را انتخاب کنید")
        return
    if messagebox.askyesno("تأیید", "آیا از حذف این قسط مطمئن هستید؟"):
        real_index, inst = _get_selected_installment()
        if real_index is None:
            return
        del data["installments"][real_index]
        save_data(data)
        update_installments_list()

tk.Button(installment_form, text="➕ ثبت قسط", command=add_installment,
          bg=settings["btn_purple"], fg='white', font=("Arial", 11), padx=20, pady=5).grid(row=2, column=0, columnspan=4, pady=10)

columns_installment = ("عنوان", "مبلغ", "تاریخ سررسید", "وضعیت", "تاریخ پرداخت")
tree_installment = ttk.Treeview(frame_installment, columns=columns_installment, show="headings", height=10)
for col in columns_installment:
    tree_installment.heading(col, text=col)
tree_installment.column("عنوان", width=200, anchor="center")
tree_installment.column("مبلغ", width=120, anchor="center")
tree_installment.column("تاریخ سررسید", width=120, anchor="center")
tree_installment.column("وضعیت", width=120, anchor="center")
tree_installment.column("تاریخ پرداخت", width=120, anchor="center")
tree_installment.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)

inst_btn_frame = tk.Frame(frame_installment, bg=settings["bg_color"])
inst_btn_frame.pack(pady=5)
tk.Button(inst_btn_frame, text="✅ ثبت پرداخت", command=mark_installment_paid,
          bg=settings["btn_green"], fg='white', font=("Arial", 10)).pack(side=tk.LEFT, padx=5)
tk.Button(inst_btn_frame, text="🗑 حذف قسط", command=delete_installment,
          bg=settings["btn_red"], fg='white', font=("Arial", 10)).pack(side=tk.LEFT, padx=5)

# ===== یادداشت‌ها =====
frame_notes = tk.Frame(notebook, bg=settings["bg_color"])
notebook.add(frame_notes, text="📝 یادداشت‌ها")

notes_header = tk.Frame(frame_notes, bg=settings["bg_color"])
notes_header.pack(fill=tk.X, padx=10, pady=5)

lbl_notes_month = tk.Label(notes_header, text="", bg=settings["bg_color"],
                            font=("Arial", 12, "bold"), fg=settings["btn_blue"])
lbl_notes_month.pack(side=tk.LEFT, padx=5)

notes_text = tk.Text(frame_notes, font=("Arial", 12), bg=settings["table_bg"],
                     wrap=tk.WORD, padx=10, pady=10)
notes_text.pack(fill=tk.BOTH, expand=True, padx=10, pady=5)

notes_btn_frame = tk.Frame(frame_notes, bg=settings["bg_color"])
notes_btn_frame.pack(pady=5)

def save_notes():
    key = get_month_key()
    content = notes_text.get("1.0", tk.END).rstrip()
    if content:
        data["notes"][key] = content
    elif key in data["notes"]:
        del data["notes"][key]
    save_data(data)
    messagebox.showinfo("موفق", "یادداشت ذخیره شد")

def update_notes():
    key = get_month_key()
    notes_text.delete("1.0", tk.END)
    content = data["notes"].get(key, "")
    if content:
        notes_text.insert("1.0", content)
    month_label = f"یادداشت‌های {filter_month.get()} {filter_year.get()}"
    lbl_notes_month.config(text=month_label)

tk.Button(notes_btn_frame, text="💾 ذخیره یادداشت", command=save_notes,
          bg=settings["btn_blue"], fg='white', font=("Arial", 11), padx=20, pady=5).pack(side=tk.LEFT, padx=5)
tk.Button(notes_btn_frame, text="🗑 پاک کردن", command=lambda: notes_text.delete("1.0", tk.END),
          bg=settings["btn_red"], fg='white', font=("Arial", 10), padx=15, pady=5).pack(side=tk.LEFT, padx=5)

tk.Label(frame_notes, text="💡 یادداشت‌ها ماه به ماه جداگانه ذخیره می‌شن",
         bg=settings["bg_color"], font=("Arial", 9, "italic"), fg="gray").pack(pady=3)

# ===== گزارش سالانه =====
frame_yearly = tk.Frame(notebook, bg=settings["bg_color"])
notebook.add(frame_yearly, text="📊 گزارش سالانه")

tree_yearly = ttk.Treeview(frame_yearly, columns=("ماه", "مجموع هزینه", "مجموع حقوق", "مجموع پس‌انداز"), show="headings", height=10)
tree_yearly.heading("ماه", text="ماه")
tree_yearly.heading("مجموع هزینه", text="مجموع هزینه (تومان)")
tree_yearly.heading("مجموع حقوق", text="مجموع حقوق (تومان)")
tree_yearly.heading("مجموع پس‌انداز", text="مجموع پس‌انداز (تومان)")
tree_yearly.column("ماه", width=120, anchor="center")
tree_yearly.column("مجموع هزینه", width=180, anchor="center")
tree_yearly.column("مجموع حقوق", width=180, anchor="center")
tree_yearly.column("مجموع پس‌انداز", width=180, anchor="center")
tree_yearly.pack(padx=10, pady=10, fill=tk.BOTH, expand=True)

def update_yearly_report():
    for row in tree_yearly.get_children():
        tree_yearly.delete(row)

    year = int(filter_year.get())
    for month_num, month_name in enumerate(jalali_months, start=1):
        start_date = jdatetime.date(year, month_num, 1)
        end_date = jdatetime.date(year, month_num, get_last_day_of_month(year, month_num))

        total_expense = sum(t["amount"] for t in data["transactions"] if _in_range(t["date"], start_date, end_date))
        total_salary = sum(s["amount"] for s in data["salary"] if _in_range(s["date"], start_date, end_date))
        total_savings = sum(sv["amount"] for sv in data["savings"] if _in_range(sv["date"], start_date, end_date))

        tree_yearly.insert("", tk.END, values=(
            month_name, f"{total_expense:,}", f"{total_salary:,}", f"{total_savings:,}"
        ))

tk.Button(frame_yearly, text="🔄 بروزرسانی گزارش سالانه", command=update_yearly_report,
          bg=settings["btn_blue"], fg='white', font=("Arial", 11)).pack(pady=5)

# ===== تنظیمات =====
frame_settings = tk.Frame(notebook, bg=settings["bg_color"])
notebook.add(frame_settings, text="⚙️ تنظیمات")

settings_frame = tk.Frame(frame_settings, bg=settings["bg_color"])
settings_frame.pack(pady=20, padx=20, anchor="center")

auto_date_var = tk.BooleanVar(value=settings["auto_date"])
tk.Checkbutton(settings_frame, text="ثبت خودکار تاریخ امروز", variable=auto_date_var, bg=settings["bg_color"], font=("Arial", 11)).grid(row=0, column=0, columnspan=2, pady=10, sticky='w')

def toggle_auto_date():
    settings["auto_date"] = auto_date_var.get()
    save_settings(settings)
    messagebox.showinfo("تنظیمات", "تنظیمات ذخیره شد")

tk.Button(settings_frame, text="ذخیره تنظیمات تاریخ", command=toggle_auto_date, bg=settings["btn_green"], fg='white').grid(row=1, column=0, columnspan=2, pady=10)

add_salary_var = tk.BooleanVar(value=settings["add_salary_to_balance"])
tk.Checkbutton(settings_frame, text="اضافه کردن حقوق به موجودی و کسر هزینه‌ها از موجودی",
               variable=add_salary_var, bg=settings["bg_color"], font=("Arial", 11)).grid(row=2, column=0, columnspan=2, pady=10, sticky='w')

def toggle_add_salary():
    settings["add_salary_to_balance"] = add_salary_var.get()
    save_settings(settings)
    messagebox.showinfo("تنظیمات", "تنظیمات ذخیره شد")

tk.Button(settings_frame, text="ذخیره تنظیمات موجودی", command=toggle_add_salary, bg=settings["btn_green"], fg='white').grid(row=3, column=0, columnspan=2, pady=10)

def change_bg_color():
    color = colorchooser.askcolor(title="انتخاب رنگ پس‌زمینه")[1]
    if color:
        settings["bg_color"] = color
        save_settings(settings)
        messagebox.showinfo("تنظیمات", "برای اعمال تغییرات، برنامه را مجدد اجرا کنید")

tk.Button(settings_frame, text="تغییر رنگ پس‌زمینه", command=change_bg_color, bg=settings["btn_orange"], fg='white').grid(row=4, column=0, pady=10, padx=5)

def change_table_color():
    color = colorchooser.askcolor(title="انتخاب رنگ جدول")[1]
    if color:
        settings["table_bg"] = color
        save_settings(settings)
        messagebox.showinfo("تنظیمات", "برای اعمال تغییرات، برنامه را مجدد اجرا کنید")

tk.Button(settings_frame, text="تغییر رنگ جدول", command=change_table_color, bg=settings["btn_blue"], fg='white').grid(row=4, column=1, pady=10, padx=5)

tk.Button(settings_frame, text="تغییر رمز عبور", command=change_password_dialog,
          bg=settings["btn_purple"], fg='white', font=("Arial", 10), padx=15, pady=5).grid(row=5, column=0, columnspan=2, pady=10)

tk.Button(settings_frame, text="فراموشی رمز عبور", command=show_forgot_password_dialog,
          bg=settings["btn_red"], fg='white', font=("Arial", 10), padx=15, pady=5).grid(row=6, column=0, columnspan=2, pady=10)

def delete_all_data():
    if messagebox.askyesno("هشدار", "آیا از حذف تمام اطلاعات مطمئن هستید؟"):
        empty = {
            "transactions": [], "salary": [], "savings": [], "charity": [],
            "debt_claim": [], "installments": [], "balance": 0,
            "initial_balance": 0, "monthly_balances": {}, "notes": {}
        }
        data.clear()
        data.update(empty)
        save_data(data)
        for fn in [update_transactions, update_salary_list, update_savings_list, update_charity_list,
                   update_debt_claim_list, update_monthly_report, update_yearly_report,
                   update_installments_list, update_balance, update_notes]:
            fn()
        messagebox.showinfo("موفق", "تمام اطلاعات حذف شد")

tk.Button(settings_frame, text="🗑 حذف تمام اطلاعات", command=delete_all_data, bg=settings["btn_red"], fg='white').grid(row=7, column=0, columnspan=2, pady=20)

tk.Label(frame_settings, text="Made By Amir Mohammad - ver 2.1", bg=settings["bg_color"],
         font=("Arial", 10, "italic", "bold"), fg="black").pack(side=tk.BOTTOM, pady=10)

def deleted_selected(tree, data_list, data_type=None):
    selected = tree.selection()
    if not selected:
        messagebox.showerror("خطا", "لطفاً یک ردیف انتخاب کنید")
        return
    if messagebox.askyesno("تأیید", "از حذف این ردیف مطمئن هستید؟"):
        index = tree.index(selected[0])
        deleted_item = data_list[-(index + 1)]

        balance = get_current_month_balance()
        if data_type == "transaction":
            balance += deleted_item["amount"]
        elif data_type == "salary":
            balance -= deleted_item["amount"]
        elif data_type == "savings":
            balance += deleted_item["amount"]
        elif data_type == "charity":
            balance += deleted_item["amount"]
        set_current_month_balance(balance)

        del data_list[-(index + 1)]
        save_data(data)

        if tree == tree_trans:
            update_transactions()
            update_monthly_report()
            update_yearly_report()
        elif tree == tree_salary:
            update_salary_list()
        elif tree == tree_savings:
            update_savings_list()
        elif tree == tree_charity:
            update_charity_list()
        elif tree == tree_dc:
            update_debt_claim_list()
        elif tree == tree_installment:
            update_installments_list()

        update_balance()
        messagebox.showinfo("موفق", "ردیف حذف شد")

def on_enter_pressed(event):
    current_tab = notebook.index(notebook.select())
    checks = [
        (entry_title, entry_amount, add_transaction),
        (entry_salary_source, entry_salary, add_salary),
        (entry_savings, None, add_savings),
        (entry_charity_name, entry_charity_amount, add_charity),
        (entry_person, entry_dc_amount, add_debt_claim),
        (entry_installment_title, entry_installment_amount, add_installment),
    ]
    if current_tab < len(checks):
        first, second, fn = checks[current_tab]
        if first.get().strip() and (second is None or second.get().strip()):
            fn()
        else:
            messagebox.showwarning("خطا", "لطفاً فیلدهای لازم را پر کنید")

root.bind("<Return>", on_enter_pressed)

if "installments" not in data:
    data["installments"] = []
if "monthly_balances" not in data:
    data["monthly_balances"] = {}
if "notes" not in data:
    data["notes"] = {}
save_data(data)

update_transactions()
update_salary_list()
update_savings_list()
update_charity_list()
update_debt_claim_list()
update_monthly_report()
update_yearly_report()
update_installments_list()
update_notes()
update_balance()

if __name__ == "__main__":
    root.mainloop()